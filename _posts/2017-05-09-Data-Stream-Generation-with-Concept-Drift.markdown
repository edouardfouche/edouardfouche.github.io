...
- If the vector is an outlier in subspace \\(S_{k}\\). We scale the value of such outlier in \\(S_{k}\\) to make sure that it is a certain distance from the margin (10% of the side of the hypercube).
- If the vector is not an outlier in subspace \\(S_{k}\\), it is moved uniformly into the margins.  

For a number of step \\(nstep\\), the set \\(S\\) is modified randomly and the value in \\(M\\) incremented/decremented to reach after a number of iteration *n* the value fixed at the next step. The result is a data stream where hypercubes of variable size in selected subspaces are formed, which potentially contain outliers. Note that the probability to find outliers in the hypercubes is proportional to the volume of the hypercube.
...
Such subspace obtains very low correlation estimates with traditional correlation measures (Pearson, Spearman, Kendall), in the order of \\(10^{-2}\\). Nonetheless, a dependence obviously exists and there is space - for example in the center - to place hidden outliers.
